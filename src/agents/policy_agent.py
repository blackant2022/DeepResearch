"""
src/agents/policy_agent.py — 统一策略 Agent（Workflow 之外的唯一决策节点）

把图级 ReAct（think ↔ tools 节点跳转）收敛为节点内策略循环。
"""
from __future__ import annotations

from config.settings import settings
from src.agents.super_agent import SuperAgent
from src.llm.provider import llm
from src.memory.context_manager import context_manager
from src.memory.long_term_memory import ltm
from src.memory.working_memory import WorkingMemory
from src.orchestrator.nodes.tool_node import execute_tool_calls
from src.rag.confidence import (
    apply_answer_confidence_gate,
    extract_knowledge_hits_from_messages,
)
from src.rag.kb_utils import is_kb_catalog_question
from src.utils.logger import get_logger

log = get_logger("policy_agent")


def _count_knowledge_searches(messages: list[dict]) -> int:
    n = 0
    for m in messages:
        if m.get("role") != "assistant":
            continue
        for tc in m.get("tool_calls") or []:
            if (tc.get("function") or {}).get("name") == "knowledge_search":
                n += 1
    return n


def _trim_knowledge_searches(tool_calls: list[dict], already: int, limit: int) -> list[dict]:
    """保留非检索工具；knowledge_search 仅保留额度内的。"""
    kept: list[dict] = []
    kb = already
    for tc in tool_calls:
        name = (tc.get("function") or {}).get("name", "")
        if name == "knowledge_search":
            if kb >= limit:
                continue
            kb += 1
        kept.append(tc)
    return kept


class PolicyAgent:
    """统一策略决策：一次图节点完成多轮工具调用。"""

    def __init__(self, wm: WorkingMemory | None = None) -> None:
        self.wm = wm or WorkingMemory()
        self.thinker = SuperAgent(self.wm)

    def run(
        self,
        question: str,
        memory_hint: str = "",
        max_iterations: int | None = None,
    ) -> dict:
        max_iter = max_iterations or settings.MAX_REACT_ITERATIONS
        max_kb = getattr(settings, "MAX_KNOWLEDGE_SEARCHES", 2)
        messages = self.thinker.build_initial_messages(question, memory_hint)
        if is_kb_catalog_question(question):
            messages.append({
                "role": "user",
                "content": "这是知识库目录类问题：请优先调用 kb_overview，不要用 knowledge_search。",
            })
        if any(k in question for k in ("百度", "baidu", "百度搜索")):
            messages.append({
                "role": "user",
                "content": (
                    "用户要求百度搜索：请优先调用名称含 baidu 的 MCP 工具"
                    "（如 baidu_baidu_web_search），不要用 web_search。"
                ),
            })

        trace: list[dict] = []
        answer = ""
        iteration = 0

        for iteration in range(1, max_iter + 1):
            already_kb = _count_knowledge_searches(messages)
            if already_kb >= max_kb:
                messages.append({
                    "role": "user",
                    "content": (
                        f"knowledge_search 已达上限（{max_kb} 次）。"
                        "请基于已有结果直接最终作答，禁止再调用 knowledge_search。"
                    ),
                })

            safe = context_manager.prepare_for_policy(messages)
            result = self.thinker.think_with_tools(safe)
            tool_calls = list(result.get("tool_calls") or [])
            assistant_msg = dict(result["assistant_message"])

            if tool_calls:
                kept = _trim_knowledge_searches(tool_calls, already_kb, max_kb)
                dropped = len(tool_calls) - len(kept)
                if dropped:
                    trace.append({
                        "step": "policy_limit",
                        "detail": f"抑制 {dropped} 次超额 knowledge_search（上限 {max_kb}）",
                    })
                if kept:
                    assistant_msg["tool_calls"] = kept
                else:
                    assistant_msg.pop("tool_calls", None)
                    if not (assistant_msg.get("content") or "").strip():
                        assistant_msg["content"] = None
                    tool_calls = []
                    messages.append(assistant_msg)
                    messages.append({
                        "role": "user",
                        "content": "请根据已有工具结果直接给出最终回答，不要再调用工具。",
                    })
                    from src.llm.messages import sanitize_for_plain_chat
                    answer = llm.chat(sanitize_for_plain_chat(messages))
                    trace.append({"step": "policy", "detail": "检索次数达上限，强制作答"})
                    break
                tool_calls = kept

            messages.append(assistant_msg)

            if not tool_calls:
                answer = result.get("content") or assistant_msg.get("content") or ""
                if not answer:
                    from src.llm.messages import sanitize_for_plain_chat
                    answer = llm.chat(sanitize_for_plain_chat(messages + [{
                        "role": "user",
                        "content": "请给出最终回答。",
                    }]))
                trace.append({
                    "step": "policy",
                    "detail": f"第 {iteration} 轮策略决策 → 最终回答",
                })
                break

            tool_names = [
                (tc.get("function") or {}).get("name", "?") for tc in tool_calls
            ]
            trace.append({
                "step": "policy",
                "detail": (
                    f"第 {iteration} 轮策略决策 → 调用 {len(tool_calls)} 个工具"
                    f"（{', '.join(tool_names)}）"
                ),
            })
            tool_msgs, tool_trace = execute_tool_calls(tool_calls, self.wm)
            messages.extend(tool_msgs)
            trace.extend(tool_trace)
        else:
            log.warning(f"策略环达到上限 {max_iter}，兜底生成")
            from src.llm.messages import sanitize_for_plain_chat

            plain = sanitize_for_plain_chat(messages)
            plain.append({
                "role": "user",
                "content": "请根据以上工具结果直接给出最终回答，不要再调用工具。",
            })
            answer = llm.chat(plain)
            trace.append({"step": "policy_finalize", "detail": "达到轮次上限，兜底生成"})

        if answer:
            used_kb = _count_knowledge_searches(messages) > 0
            hits = extract_knowledge_hits_from_messages(messages)
            if used_kb and settings.ANSWER_CONFIDENCE_ENABLED:
                answer, conf_report = apply_answer_confidence_gate(
                    answer, hits, used_knowledge=True, query=question
                )
                detail = (
                    f"置信度 {conf_report.get('confidence')} "
                    f"< {conf_report.get('threshold')}，已输出兜底提示"
                    if conf_report.get("fallback")
                    else f"置信度 {conf_report.get('confidence')} 通过"
                )
                trace.append({
                    "step": "confidence_gate",
                    "detail": detail,
                    "meta": conf_report,
                })

            ltm.consolidate(f"问题：{question}\n结论：{answer[:400]}", question)
            trace.append({"step": "policy_done", "detail": "策略 Agent 作答完成"})

        return {
            "final_answer": answer,
            "messages": messages,
            "react_iteration": iteration,
            "trace": trace,
            "wm_items": self.wm.export_snapshot(),
        }
